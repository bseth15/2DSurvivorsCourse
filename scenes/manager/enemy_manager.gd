...
# Just larger than half the view port width so that enemies spawn off-screen.
...
func get_spawn_position() -> Vector2:
	...
	for i in 4:
		...
		var query_parameters = PhysicsRayQueryParameters2D\
			.create(player.global_position, spawn_position + additional_check_offset, 1 << 0)
...
func on_timer_timeout():
	...
	while spawn_position == Vector2.ZERO:
		# Work around for spawning outside of map,
		# will probably bite me in the ass some day.
