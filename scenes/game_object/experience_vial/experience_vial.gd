...
func on_area_entered(other_area: Area2D):
	# trying to disable a physic node within a physics call back raises an error
	# call function outside of physic process (the next idle frame)
	Callable(disable_collision).call_deferred()

	var tween = create_tween()
	tween.set_parallel()  # run all tweeners in parallel
	...
	tween.chain()  # wait for previous tweeners to finish before continuing
